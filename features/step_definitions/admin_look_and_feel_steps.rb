...
Given(/^community "(.*?)" has default browse view "(.*?)"$/) do |community, browse_view|
  Community.where(username: community).first.update_attributes(default_browse_view: browse_view)
end
...
Given(/^community "(.*?)" has name display type "(.*?)"$/) do |community, name_display_type|
  Community.where(username: community).first.update_attributes(name_display_type: name_display_type)
end
