...
Given(/^that listing belongs to community "(.*?)"$/) do |username|
  @listing.communities = [Community.where(username: username).first]
end

Given /^that listing is visible to members of community "([^"]*)"$/ do |username|
  @listing.communities << Community.where(username: username).first
end
...
Given /^there is a dropdown field "(.*?)" for category "(.*?)" in community "(.*?)" with options:$/ do |field_title, category_name, community_username, opts_table|
...
Given /^listing comments are in use in community "(.*?)"$/ do |community_username|
  community = Community.where(username: community_username).first
