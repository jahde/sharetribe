module CommunitySteps
  ...
  def use_payment_gateway(community_username, gateway_name, commission)
    gateway_name ||= "Checkout"
    commission ||= "8"

    community = Community.where(username: community_username).first
...
Given /^there are following communities:$/ do |communities_table|
  communities_table.hashes.each do |hash|
    username = hash[:community]
    existing_community = Community.where(username: username).first
    existing_community.destroy if existing_community
    @hash_community = FactoryGirl.create(:community, :username => username, :settings => {"locales" => ["en", "fi"]})
...
Given /^the terms of community "([^"]*)" are changed to "([^"]*)"$/ do |community, terms|
  Community.where(username: community).first.update_attribute(:consent, terms)
end

Given /^"(.*?)" is a member of community "(.*?)"$/ do |username, community_name|
  community = Community.where(username: community_name).first
...
Then /^Most recently created user should be member of "([^"]*)" community with(?: status "(.*?)" and)? its latest consent accepted(?: with invitation code "([^"]*)")?$/ do |community_username, status, invitation_code|
...
Given /^given name and last name are not required in community "([^"]*)"$/ do |community|
  Community.where(username: community).first.update_attribute(:real_name_required, 0)
end

Given /^community "([^"]*)" requires invite to join$/ do |community|
  Community.where(username: community).first.update_attribute(:join_with_invite_only, true)
end

Given /^community "([^"]*)" does not require invite to join$/ do |community|
  Community.where(username: community).first.update_attribute(:join_with_invite_only, false)
end

Given /^community "([^"]*)" requires users to have an email address of type "(.*?)"$/ do |community, email|
  Community.where(username: community).first.update_attribute(:allowed_emails, email)
end

Given /^the community has payments in use(?: via (\w+))?(?: with seller commission (\w+))?$/ do |gateway_name, commission|
  use_payment_gateway(@current_community.username, gateway_name, commission)
end

Given /^community "([^"]*)" has payments in use(?: via (\w+))?(?: with seller commission (\w+))?$/ do |community_username, gateway_name, commission|
  use_payment_gateway(community_username, gateway_name, commission)
end

Given /^users (can|can not) invite new users to join community "([^"]*)"$/ do |verb, community|
  can_invite = verb == "can"
  Community.where(username: community).first.update_attribute(:users_can_invite_new_users, can_invite)
end

Given /^there is an invitation for community "([^"]*)" with code "([^"]*)"(?: with (\d+) usages left)?$/ do |community, code, usages_left|
  inv = Invitation.new(:community => Community.where(username: community).first, :code => code, :inviter_id => @people.first[1].id)
...
When /^I move to community "([^"]*)"$/ do |community|
  ...
  @current_community = Community.where(username: community).first
end
...
Then /^community "(.*?)" should require invite to join$/ do |community|
   Community.where(username: community).first.join_with_invite_only.should be_truthy
end

Then /^community "(.*?)" should not require invite to join$/ do |community|
   Community.where(username: community).first.join_with_invite_only.should_not be_truthy
end

Given /^community "(.*?)" is private$/ do |community_username|
  Community.where(username: community_username).first.update_attributes({:private => true})
end
...
Given /^listing publishing date is shown in community "(.*?)"$/ do |community_username|
  Community.where(username: community_username).first.update_attributes({:show_listing_publishing_date => true})
end
